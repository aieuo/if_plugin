...
class ChainIfForm {
    ...
    public function onAddChainIf($player, $ifData) {
        $session = Session::getSession($player);
        if ($ifData === null) {
        ...
        $session->set("if_key", $ifData[0]);
        $data = $manager->repairIF([]);
        $manager->set($ifData[0], $data);
    ...
    public function onEditChainIf($player, $ifData) {
        $session = Session::getSession($player);
        if ($ifData === null) {
            $session->setValid(false, false);
            return;
        ...
        if ($action === "edit") {
            $data = $manager->repairIF($manager->get($ifData[0]));
            $mes = IFAPI::createIFMessage($data["if"], $data["match"], $data["else"]);
            $form = (new Form)->getEditIfForm($mes, $data["name"] ?? null);
            Form::sendForm($player, $form, new Form(), "onEditIf");
        } elseif ($action === "del") {
    ...
    public function onChainIfList($player, $ifData) {
        $session = Session::getSession($player);
        if ($ifData === null) {
            $session->setValid(false, false);
            return;
        ...
        $manager = IFPlugin::getInstance()->getChainManager();
        $ifs = array_slice($manager->getAll(), $ifData-1, 1, true);
        $key = key($ifs);
        $data = current($ifs);
